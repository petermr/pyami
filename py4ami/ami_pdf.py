...
class AmiPage:
    ...
    def create_html_pages(cls,
        ...
        if not input_pdf or not Path(input_pdf).exists():
            logging.getLogger().error(f"must have not-null, existing pdf {input_pdf} ")
        ...
        #     logging.error(f"range list {range_list}")
        #     print(f"page 1-based {page_1based}; range list {range_list}")
            if not Util.range_list_contains_int(page_1based, range_list):
                continue
            html = AmiPage.chars_to_spans(bbox, input_pdf, page_no)
            output_html = Path(output_dir, f"{output_stem}_{page_1based}.html")
...
#        logging.warning(f"ARG DICT {self.arg_dict}")
        ...
        # self.convert_write(maxpages=self.maxpage, outdir=self.outdir, outstem=self.outstem, inpath=self.inpath, flow=True)
        self.convert_write()
